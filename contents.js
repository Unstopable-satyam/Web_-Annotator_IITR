...
document.addEventListener('click', function (event) {
  ...
  if (!document.querySelector('.hover-option-button') && highlightedText.classList.contains('highlight')) {
    ...
    button1.src = 'https://static-00.iconduck.com/assets.00/edit-icon-1022x1024-kes437mc.png'
    button2.src = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQyCGKJlEQKUOcWMWM3Wwgy08DniWXJTP6SCQ&s'
    button3.src = 'https://i.pinimg.com/originals/7d/0f/5c/7d0f5cb6ae2e74ad2f8f8c4789379a3a.png'
    button1.classList.add('btn-img')
    button2.classList.add('btn-img')
    button3.classList.add('btn-img')
    button1.height = '30'
    button1.width = '30'
    button2.height = '30'
    button2.width = '30'
...
function highlightText(colour) {
  ...
  function calculateOffsets(range, parentElement) {
    ...
    while (
      startContainerHTML[startContainerHTML.length - 1] == ' ' ||
      startContainerHTML[startContainerHTML.length - 1] == String.fromCharCode(160)
    ) {
    ...
    while (
      endContainerHTML[endContainerHTML.length - 1] == ' ' ||
      endContainerHTML[endContainerHTML.length - 1] == String.fromCharCode(160)
    ) {
    ...
    for (let i = 0; i < htmlContent.length; i++) {
      ...
      if (i == htmlContent.indexOf(startContainerHTML) && check == 1) {
        ...
        htmlContent = htmlContent.substring(
          htmlContent.indexOf(startContainerHTML) + startContainerHTML.length,
          htmlContent.length
        )
        i = 0
...
function restoreHighlight({
  ...
  if (inner[startOffset] == '>' && (text[0] == '<' || text[1] == '<') && temp > startOffset) {
    ...
    inner =
      ...
      text +
      '</span> ' +
      inner.substring(text.length + startOffset - x - 1, inner.length)
  } else {
    ...
    if (issue)
      inner =
        ...
        inner.substring(text.length + startOffset - tag2.length, inner.length)
    else
      inner =
        inner.substring(0, startOffset) +
        "<span class='highlight' style='background-color:" +
        ...
        text +
        '</span>' +
        inner.substring(text.length + startOffset, inner.length)
  }
  parent.innerHTML = inner
